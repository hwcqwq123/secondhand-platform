...
在商品发布流程中，前端通常会先上传商品图片，再提交商品标题、价格、描述、分类等商品信息。

原来的逻辑存在一个问题：


图片上传成功后，图片文件已经保存到后端 uploads/items 目录；
但是如果后续商品发布失败、用户清空表单、用户删除刚上传的图片，或者用户直接关闭浏览器；
这些图片可能不会被商品引用；
长期使用后，uploads/items 目录中会残留大量无效图片文件。

## 管理员端功能完善说明

本次修改主要针对管理员端权限、商品删除约束、订单管理、用户管理、操作日志和后台数据统计进行了完善。

---

## 16. 功能更新说明

本节用于记录项目后续新增和优化的主要功能。

---

### 16.1 未登录浏览与下单登录优化

平台支持未登录用户直接进入首页浏览商品，也可以查看商品详情。

当未登录用户点击下单、发布商品、查看我的订单、查看个人中心等需要登录的功能时，系统会弹出登录或注册窗口，提示用户先登录后再继续操作。

该功能提升了用户浏览体验，使用户无需登录也能先了解平台商品内容，同时保证下单、发布、订单管理等关键操作仍然需要登录权限。
...
- 防止用户购买自己发布的商品；
- 订单取消后，商品可以恢复为可购买状态；
- 支付和订单状态由后端进行判断，避免只依赖前端倒计时；
- 下单失败后，前端会刷新商品状态，避免显示状态和实际状态不一致；
- 商品删除时会考虑订单关联，避免影响已有交易记录；
- 推荐商品会过滤已删除、已下架、已售出和用户自己发布的商品。

这些优化提高了交易流程的可靠性，减少了并发下单、状态不同步和订单数据异常的问题。

---

### 16.3 Redis 并发下单优化

项目引入 Redis 对下单流程进行并发控制。

当多个用户同时下单同一件商品时，系统会先通过 Redis 对商品进行加锁，只有第一个成功获取锁的请求可以继续进入后端订单创建流程。

Redis 主要用于减少并发请求直接进入数据库，MySQL 事务和商品行锁负责最终的数据一致性。
...
↓
释放 Redis 锁
通过该优化，可以保证同一件商品在同一时间只能被一个用户成功下单。
```

## 16.4 图片上传无效文件清理

商品发布过程中，用户可能先上传图片，但后续没有成功发布商品，或者删除了已上传图片。

为了避免 uploads/items 目录长期残留无效图片，系统增加了图片清理逻辑。

该功能可以减少无效文件堆积，避免上传目录越来越大，也方便后续维护商品图片资源。

## 16.5 管理员端功能完善

后台管理端增加了更加完整的平台管理能力，主要包括：
...
管理员可以在后台查看平台商品、订单、用户和统计数据，并对异常数据进行处理。

对于删除商品、修改订单状态、封禁用户等重要操作，系统会要求管理员确认并填写操作原因，方便后续追踪和管理。

## 16.6 管理员注册功能

后台管理端新增管理员注册功能。

在管理后台登录页面中，“登录后台”按钮下方新增了“注册管理员账号”按钮。点击后可以进入管理员注册页面，注册普通管理员或分工管理员。

管理员注册无需邀请码，注册成功后可以自动登录并进入后台。
...
不能注册 admin 用户名；
不能注册 SUPER_ADMIN 角色；
不能把其他用户修改为 SUPER_ADMIN；
不能封禁固定超级管理员 admin；
不能修改固定超级管理员 admin 的角色。
...
该功能解决了管理员账号只能手动修改数据库生成的问题，使后台管理员账号管理更加规范。

## 16.7 管理端 UI 自适应优化

后台管理端界面增加了响应式自适应布局。

页面会根据屏幕大小自动调整排版，适配大屏电脑、普通笔记本、平板和小窗口浏览器。
...
该功能提升了后台页面在不同屏幕尺寸下的显示效果，使管理端排版更加美观、整齐。