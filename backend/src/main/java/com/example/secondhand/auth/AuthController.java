...
public class AuthController {
  ...
  public ApiResponse<AuthResponse> register(@Valid @RequestBody RegisterRequest req) {
    if (users.existsByUsername(req.username())) return ApiResponse.fail("username already exists");
    User u = new User();
    ...
    return ApiResponse.ok(new AuthResponse(token, u.getId(), u.getUsername()), "registered");
  }
  ...
  public ApiResponse<AuthResponse> login(@Valid @RequestBody LoginRequest req) {
    ...
    String token = jwtService.generateToken(u.getUsername(), u.getRole());
    return ApiResponse.ok(new AuthResponse(token, u.getId(), u.getUsername()), "logged in");
  }
}
